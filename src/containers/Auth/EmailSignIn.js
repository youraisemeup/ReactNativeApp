...
class EmailSignIn extends Component {
  ...
  componentDidMount() {
    if (this.props.isSigned) {
      this.props.navigation.navigate('web_cellphone');
    }
  }
  ...
  onLogin() {
    ...
    const nameError = validator('name', this.state.name);
    const phoneError = validator('phone', this.state.phoneNumber);
    // console.log('err: ', emailError, passwordError, nameError, phoneError);
    if (emailError) {
      this.setState({
        errorType: 'email'
      });
  ...
  render() {
    const { name, email, phoneNumber, password, errorType } = this.state;
